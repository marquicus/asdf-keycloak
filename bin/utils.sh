...
download_path() {
  ...
  echo "$tmp_download_dir/keycloak-${version}.tar.gz"
}
...
download_url() {
  ...
  if [ "$install_type" = "version" ]; then
    ...
    echo "https://github.com/keycloak/keycloak/archive/${version}.tar.gz"
  fi
...
download_keycloak() {
  ...
  if [ "$TMPDIR" = "" ]; then
    tmp_download_dir=$(mktemp -d -t keycloak_build_XXXXXX)
  else
